...
function AllProducts(props) {
    ...
    const { cat, addToCart, addToFavorites } = props
    ...
    return (
        <div className='page-container'>
            {prod.length}{prod.map((card, index) => {
                return (<div className='product-card' key={index}>
                    ...
                    <div className='product-details'>
                        ...
                        <button className={` ${props.cart.includes(card)} 'addtocartAdded-class' : 'addtocart'}`} onClick={() => addToCart(card)} >{props.cart.includes(card)}<div><span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="14" fill="currentColor" class="bi bi-cart-x-fill" viewBox="0 0 16 16">
